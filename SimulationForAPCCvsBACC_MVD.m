...
Delta2 = 5; %search  interval around the value of VectorK_i_MVD_T2
Delta3 = 12;
Mark_simulation = 1;
Mark_BF = 1; %decide whether to perform Brute-force algorithm.
...
for index_r = 1:length(Vector_r) %the maximum of r should be determined carefully later
    ...
    for index1 = 1:length(VectorKPrime)
        ...
        %initilization
        minK_i = max(2,VectorK_i_MVD_T2(end)-Delta2);
        maxK_i = min([floor(K/r), KPrimeMax + 1, VectorK_i_MVD_T2(end)+Delta2]);
        VectorK_i = zeros(1,r);
        DelayAPCCMinTemp = alpha0 + 1/mu0;%max(sum(VectorTm));
        VectorK_iMinTemp = VectorK_i_MVD_T2;

        %       % decide whether to run this part of codes
        if Mark_BF == 1 && r == r_BF
            tic
            [~, VectorK_iMinTemp] = LoopSearch_wo_Cancellation(minK_i,maxK_i,VectorK_i,r,K,DelayAPCCMinTemp,VectorK_iMinTemp,RepeatTimes2,N,d,L,K,mu0,alpha0,VectorK_i_MVD_T2,Delta2);
            ExcuTimeSimuOpt_T2 = ExcuTimeSimuOpt_T2 + toc;
        end

        VectorK_i_SimuOpt_T2 = VectorK_iMinTemp;
        ...
        [VectorKStar_i, Flag, ~] = T3BasedK_i(H,N,d,L,mu,alpha,r,Delta1);
        ...
        tic
        [VectorK_i_MVD_T3, ~, tMVD_T3] = MVD_T3_K_i(VectorK_i_Rounding,r,K,N,d,L,alpha,mu,KPrimeMax);
        ...
        %initilization
        minK_i = max(2,VectorK_i_MVD_T3(end)-Delta3);
        maxK_i = min([floor(K/r), KPrimeMax + 1, VectorK_i_MVD_T3(end)+Delta3]);
        VectorK_i = zeros(1,r);
        DelayAPCCMinTemp = alpha0 + 1/mu0;%max(sum(VectorTm));
        VectorK_iMinTemp = VectorK_i_MVD_T3;

        %       % decide whether to run this part of codes
        if Mark_BF == 1 && r == r_BF
            tic
            [DelayAPCCMinTemp, VectorK_iMinTemp] = LoopSearch_w_Cancellation(minK_i,maxK_i,VectorK_i,r,K,DelayAPCCMinTemp,VectorK_iMinTemp,RepeatTimes2,N,d,L,K,mu0,alpha0,VectorK_i_MVD_T3,Delta3);
            ExcuTimeSimuOpt_T3 = ExcuTimeSimuOpt_T3 + toc;
        end

        VectorK_i_SimuOpt_T3 = VectorK_iMinTemp;
        ...
        for index3 = 1: RepeatTimes
            ...
            DelayAPCCTemp3 = ComputeEndDelay_wCancel(VectorTm,r,VectorEta_i_MVD_T2);
            SumDelayAPCC_Cancellation = SumDelayAPCC_Cancellation + DelayAPCCTemp3;
            if sum(abs(VectorEta_i_SimuOpt_T3 - VectorEta_i_MVD_T2)) ~= 0 && r == r_BF
...
DelayBACCMinimum = min(DelayBACC);
...
if Mark_simulation == 1
    ...
    plot(xtemp,ytemp,'LineWidth',1.25,'DisplayName',['APCC w/ cancellation, MVD, r=' num2str(Disp_r1)])

    ytemp = 1000*DelayAPCC3(DelayAPCC3 ~= 0);
    xtemp = VectorKPrime(DelayAPCC3 ~= 0);
    plot(xtemp,ytemp,'--','LineWidth',1.25,'DisplayName',['APCC w/o cancellation, Brute-Force, r=' num2str(r_BF)])

    ytemp = 1000*DelayAPCC4(DelayAPCC4 ~= 0);
    xtemp = VectorKPrime(DelayAPCC4 ~= 0);
    plot(xtemp,ytemp,'--','LineWidth',1.25,'DisplayName',['APCC w/ cancellation, Brute-Force, r=' num2str(r_BF)])
    ...
    plot(xtemp,ytemp,'LineWidth',1.25,'DisplayName',['APCC w/ cancellation, MVD, r=' num2str(Disp_r2)])
    ...
    xtemp = VectorKPrime;
    plot(xtemp,ytemp,'-','LineWidth',1.25,'DisplayName','BACC')
    ...
    legend
    %     title('APCC w/o cancellation vs BACC')
...
%     DelayList1 = NaN.*rand(2,7);
    ...
    DelayList2 = [NaN.*rand(2,3);rand(2,3)];
    ...
    LabelList = categorical({'N=500,d=25','N=700,d=35'});
    LabelList = reordercats(LabelList,{'N=500,d=25','N=700,d=35'});

    if N==500 
        DelayList1(1,1) = 1000*DelayBACCMinimum;
        DelayList1(1,2) = 1000*Vec_r_DelayAPCCMinimum_MVD_T2(Vector_r == 16);
        DelayList1(1,3) = 1000*Vec_r_DelayAPCCMinimum_MVD_T3(Vector_r == 16);
    ...
    if N==700
        DelayList1(2,1) = 1000*DelayBACCMinimum;
        DelayList1(2,2) = 1000*Vec_r_DelayAPCCMinimum_MVD_T2(Vector_r == 16);
        DelayList1(2,3) = 1000*Vec_r_DelayAPCCMinimum_MVD_T3(Vector_r == 16);
    ...
    figure(3),hold on
    set(gcf,'Units','centimeter','Position',[5 5 21 9]);
    ...
    legend('BACC', ['APCC w/o Cancel, MVD, r=',num2str(16)], ['APCC w/ Cancel, MVD, r=',num2str(16)], ...,
