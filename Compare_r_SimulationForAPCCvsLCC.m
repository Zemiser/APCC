...
Vector_r = 4:12;%the maximum of r should be determined carefully later
...
DelayAPCCMinimum_T2BH_r = (alpha0 + 1/mu0)*ones(1,length(Vector_r));
DelayAPCCMinimum_T3BH_r = (alpha0 + 1/mu0)*ones(1,length(Vector_r));
...
%         end
        ...
        VectorEta_i_Rounding_T2 = d*(VectorK_i_Rounding + L -1) +1; % to update the corresponding integer eta_i, Eta_i = VectorEta_i_Rounding(i+1);
        %         DivisionRounding{1,index_r}(:,index1) = VectorK_i_Rounding';

        Vector_t_i = zeros(1,r);% t_i = Vector_t_i(i+1),i is taken within [0:r-1]
        for index3 = 1:r
            Vector_t_i(index3) = alpha*index3 - index3*log(1 - VectorEta_i_Rounding_T2(index3)/N)/mu;
        end
...
%         t_r_T2BH(index_r,index1) = tT2BH;
        ...
        VectorK_i_Rounding = RoundingK_i(K,r,VectorK_i_Rounding,KPrimeMax);
        VectorEta_i_Rounding_T3 = d*(VectorK_i_Rounding + L -1) +1; % to update the corresponding integer eta_i, Eta_i = VectorEta_i_Rounding(i+1);
        %         DivisionRounding{1,index_r}(:,index1) = VectorK_i_Rounding';

        Vector_t_i = zeros(1,r);% t_i = Vector_t_i(i+1),i is taken within [0:r-1]
        for index3 = 1:r
            Vector_t_i(index3) = alpha*index3 + index3/mu/(N - VectorEta_i_Rounding_T3(index3) + 1);
        end
        tUB = max(Vector_t_i); % serves as an upperbound for B&B algorithm
...
%         DivisionT3BH{1,index_r}(:,index1) = VectorK_i_T3BH';
        ...
        for index3 = 1: RepeatTimes
            ...
            DelayAPCCTemp = ComputeEndDelay_woCancel(VectorTm,r,VectorEta_i_Rounding_T2); % to obtain the delay of the complete task
            SumDelayAPCC = SumDelayAPCC + DelayAPCCTemp;
            DelayAPCCTemp2 = ComputeEndDelay_wCancel(VectorTm,r,VectorEta_i_Rounding_T3);
        ...
        if DelayAPCC <= DelayAPCCMinimum_T2BH_r(index_r)
            DelayAPCCMinimum_T2BH_r(index_r) = DelayAPCC;
        end
        if DelayAPCC_Cancellation <= DelayAPCCMinimum_T3BH_r(index_r)
            DelayAPCCMinimum_T3BH_r(index_r) = DelayAPCC_Cancellation;
        end
...
ytemp = DelayAPCCMinimum_T2BH_r;
xtemp = Vector_r;
plot(xtemp,ytemp,'-','LineWidth',1.25,'DisplayName','APCC w/o cancellation')

ytemp = DelayAPCCMinimum_T3BH_r;
xtemp = Vector_r;
plot(xtemp,ytemp,'-','LineWidth',1.25,'DisplayName','APCC w/ cancellation')
...
ylabel('Average Minimum Delay (ms)')
