function [VectorKStar_i, Flag, tStar] = T3LowerBoundK_i(K_remaining,N,d,L,mu,alpha,r,tStarUB,index_r,LastK_i,Delta1)
...
VectorKStar_i = zeros(1,index_r); % K_i = VectorKStar_i(i+1);
...
if index_r == 1
    ...
    Flag = 1;
    tStar = alpha*index_r + index_r/mu/(N - VectorEtaStar_i + 1);
    return
end

minimumK_i = max(2,LastK_i);
maximumK_i = min(floor(K_remaining/index_r), KPrimeMax + 1);
countTimes = -1; % to count the number of repeated optimal solutions
for K_i = minimumK_i:maximumK_i
    Eta_i = d*(K_i + L - 1) + 1;
    tStarTemp1 = alpha*index_r + index_r/mu/(N - Eta_i + 1);
    if tStarTemp1 > tStarUB
        continue;
    else
        H_remaining = d*(K_remaining - K_i + (index_r-1)*L - (index_r-1)) + (index_r-1);
        [VectorKStar_iTemp1, FlagTemp, tStarTemp2] = T3BasedK_i(H_remaining,N,d,L,mu,alpha,index_r-1,Delta1);
    end

    if (FlagTemp == 0) || (tStarTemp2 > tStarUB)
        continue;
    else
        [VectorKStar_iTemp2, Flag, tStarTemp3] = T3LowerBoundK_i(K_remaining - K_i,N,d,L,mu,alpha,r,tStarUB,index_r-1,K_i,Delta1);
