...
% DelayAPCCMinimum_Cancellation_MVD = alpha0 + 1/mu0;
Vec_r_DelayAPCCMinimum_Cancellation_MVD = ones(1,length(Vector_r))*(alpha0 + 1/mu0);
Vec_r_DelayAPCCMinimum_Cancellation_SimuOpt = ones(1,length(Vector_r))*(alpha0 + 1/mu0);
...
for index_r = 1:length(Vector_r) %the maximum of r should be determined carefully later
    ...
    %     DivisionTest = zeros(r,length(VectorKPrime)); % The division result based on T3 with fixed r
...
%         end
        tic
        [VectorK_i_MVD, FlagMVD, tMVD] = MVD_T3_K_i(VectorK_i_Rounding,r,K,N,d,L,alpha,mu,KPrimeMax);
...
%         end
        ...
        %% To obtain the optimal K_i (SimuOpt) in simulation:APCC with cancellation
        ...
        if Mark_BF == 1 && r == r_BF
            tic
            [DelayAPCCMinTemp, VectorK_iMinTemp] = LoopSearch_w_Cancellation(minK_i,maxK_i,VectorK_i,r,K,DelayAPCCMinTemp,VectorK_iMinTemp,RepeatTimes2,N,d,L,K,mu0,alpha0,VectorK_i_MVD,Delta2);
        ...
        if Mark_simulation == 1
            ...
            for index3 = 1: RepeatTimes
                ...
                DelayAPCCTemp = ComputeEndDelay_wCancel(VectorTm,r,VectorEta_i_MVD); % to obtain the delay of the complete task
            ...
            DelayAPCC_r_MVD(index_r,index1) = DelayAPCC_Cancellation;
            DelayAPCC_r_SimuOpt(index_r,index1) = DelayAPCC_Cancellation_SimuOpt;

            if DelayAPCC_Cancellation <= Vec_r_DelayAPCCMinimum_Cancellation_MVD(index_r)
                Vec_r_DelayAPCCMinimum_Cancellation_MVD(index_r) = DelayAPCC_Cancellation;
            end
            if DelayAPCC_Cancellation_SimuOpt <= Vec_r_DelayAPCCMinimum_Cancellation_SimuOpt(index_r)
                Vec_r_DelayAPCCMinimum_Cancellation_SimuOpt(index_r) = DelayAPCC_Cancellation_SimuOpt;
            end
        end
...
[DelayAPCCMinimum_Cancellation_MVD,temp2] = min(Vec_r_DelayAPCCMinimum_Cancellation_MVD);
...
if Mark_simulation == 1
    ...
    plot(xtemp,ytemp,'-','LineWidth',1.25,'DisplayName',['APCC w/ cancellation, MVD, r=' num2str(r_Optimal_MVD)])
